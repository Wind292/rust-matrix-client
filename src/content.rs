...
impl Cache {
    ...
    pub fn debug_types(self) {
        for event in self.events.into_iter() {
            match event {
                Event::Message(e)=> {println!("Message: {}", e.body)}
                Event::Name(e)=> {println!("Name set to: {}", e.name)}
                Event::Unknown(e)=> {println!("Unknown of type: {}", e.event_type)}

            }
        }
    }

    pub async fn from_rooms(rooms: Value) -> Result<HashMap<String, (Self, Self)>, Box<dyn std::error::Error>> {
        ...
        for room in rooms.as_object().ok_or(MissingRequiredField)? {
            ...
            // This room's caches
            let state_cache = Cache { // First is State
                events: state_events,
...
fn parse_event(event_json: Value) -> Result<Event, Box<dyn std::error::Error>> {
    let event_type = event_json.get("type").and_then(|f| f.as_str()).ok_or(MissingRequiredField)?;
    Ok(match event_type {
...
impl MessageEvent {
    fn format(json: Value) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let msg_type = content.get("msgtype").and_then(|t| t.as_str()).ok_or(MissingRequiredField)?;
        let body = content.get("body").and_then(|t| t.as_str()).ok_or(MissingRequiredField)?;
        
        let format_type = content.get("format").and_then(|t| t.as_str());
        let mut format_string = content.get("formatted_body").and_then(|t| t.as_str()).and_then(|s| Some(s.to_string()));
        if format_type != Some("org.matrix.custom.html") { format_string = None } // unsupported formatting; fallbacks to body

        let sender = json.get("sender").and_then(|t| t.as_str()).and_then(|s| Some(s.to_string()));
        let event_id = json.get("event_id").and_then(|t| t.as_str()).and_then(|s| Some(s.to_string()));
        let room_id = json.get("room_id").and_then(|t| t.as_str()).and_then(|s| Some(s.to_string()));
...
impl NameEvent {
    fn format(json: Value) -> Result<Self, Box<dyn std::error::Error>> {
        let content = json.get("content").ok_or(MissingRequiredField)?; 
        let name = content.get("name").and_then(|t| t.as_str()).ok_or(MissingRequiredField)?;
...
impl UnknownEvent {
    fn format(json: Value) -> Result<Self, Box<dyn std::error::Error>> {
        let event_type = json.get("type").and_then(|t| t.as_str()).ok_or(MissingRequiredField)?;
